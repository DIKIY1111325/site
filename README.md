...
КНОПКИ
1. "VPN за несколько кликов" тариф1 = за отправленный инвайт(сначало пользователь попадает на страницу инвайт и выполняет условия страницы(описание работы стриницы INVITE ниже) после того как пользователь поделился ссылкой в мессенджере, пользователь преадресуется на страницу login и после успешной авторизации или регистрации (описание работы стриницы LOGIN смотреть ниже) на странице логин он переадсовывается на страницу vpn_client() где имеет возможность скачать по ссылкам (в appstore и googlestore  и другим) приложение клиент и/или копировать ссылку "ключ" для подключения к серверу xray ОБЯЗАТЕЛЬНОЕ УСЛОВИЕ ПОСТУПНОСТИ ПОЛУЧЕНИЯ ССЫЛКИ это выполнение условий: пользователь выполнил условия страницы инвайт+авторизован 
2. 
...
и чек боксом согласия с условиями (с ссылкой для ознакомления с условиями на странице contact)
...
Если пользователь выбрал нажав кнопку на странице индекс "Подключить VPN за символическую плату" и перешел на страницу авторизации, после успешной авторизации он должен быть перенаправлен на страницу faq3_vpn_serv.html.

Если пользователь выбрал нажав кнопку на странице индекс "Подключиться к личному VPN серверу" и перешел на страницу авторизации, после успешной авторизации он должен быть перенаправлен на страницу faq4_vpn_serv.html.
